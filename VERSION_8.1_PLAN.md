...
| 13 | Release closeout | **Done** (pending maintainer gates) | PolyPrompt 2.4.1 published 2026-09-01 and verified (both TFMs carry the Anthropic/VoyageAI clients); reference bumped; all three v8.1.0 images build from nuget.org; live compose acceptance passed (8 services healthy on PostgreSQL, smoke 17/17, VoyageAI live probe, Prometheus rest+mcp up); seed DBs regenerated on the v8.1 schema; dashboard session-token bug found in live login testing and fixed. Remaining for the maintainer: rendered walkthrough, multi-arch image push + SDK publishing, PR click (compare/main...V8.1). |
...
- [x] Live `docker compose up` acceptance. *(2026-09-01: gate cleared by PolyPrompt 2.4.1 — all three v8.1.0 images build from nuget.org packages; 8-service stack healthy on PostgreSQL; smoke.ps1 17/17 incl. chat CRUD/health/negative probes; VoyageAI live connectivity test Reachable (355ms) with background monitoring active; Prometheus scraping rest+mcp targets up; 26 chat metric lines on /metrics; seed databases regenerated with the v8.1 schema. Dashboard session-token bug found during the maintainer's live login test and fixed (see commit 73fdb88). Remaining for the maintainer: rendered walkthrough, multi-arch image push, SDK package publishing, PR click.)*
