...
- `POST /v1.0/tenants/{tenantGuid}/graphs/{graphGuid}/algorithms/embeddings` — generate node embeddings for the graph using the tenant's active embedding endpoint and store each as a node vector (HNSW-indexable). Body is a `GenerateEmbeddingsRequest` (optional `MaxNodes`, `SkipNodesWithVectors`). Requires `write`, and the chat/embedding feature enabled with an active embedding endpoint (otherwise `400`).
...
The algorithm request also accepts `UseCache` (serve/store a cached result, invalidated when the graph's node or edge count changes). Algorithms are additionally callable from the native query language via `CALL litegraph.algo.<name>() RETURN guid, score` on the `/query` route — see [DSL.md](DSL.md) and [ALGORITHMS.md](ALGORITHMS.md).
