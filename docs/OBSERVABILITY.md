...
The `algorithm` label is the `AlgorithmType` (for example `PageRank`, `Louvain`, `BetweennessCentrality`). Runs invoked through the REST `/algorithms` route (and the `algorithm/run` MCP tool, which proxies it) open the `litegraph.graph.algorithm` OpenTelemetry span, tagged with `litegraph.algorithm.type` and `litegraph.algorithm.node_count`. Algorithms invoked through the native query language (`CALL litegraph.algo.*`) are counted under the same `litegraph_graph_algorithms_total` / `litegraph_graph_algorithm_duration_ms` series (labeled by algorithm) and tag the query span with `litegraph.algorithm.type`, so DSL-driven algorithm runs appear in the dashboards alongside the REST/MCP runs. A provisioned **LiteGraph Algorithms** Grafana dashboard (`litegraph-algorithms`) shows run rate, total and failed runs, runs by algorithm type, and average duration by algorithm. It is provisioned from `assets/grafana/` (mounted by the Compose deployments), so it appears automatically under the LiteGraph folder in Grafana.
