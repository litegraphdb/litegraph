namespace LiteGraph
{
    ...
    public class ChatEndpoint
    {
        ...
        /// Redacted to its last four characters in API responses.
        /// </summary>
        ...
        /// the chat orchestrator caps the conversation-history budget so history plus output fit the window.
        ...
        /// <summary>
        /// Produce a copy of this endpoint with the API key redacted to its last four characters.
        /// </summary>
        /// <returns>Redacted copy.</returns>
        public ChatEndpoint Redact()
        {
            ChatEndpoint ret = (ChatEndpoint)MemberwiseClone();
            if (!String.IsNullOrEmpty(ret.ApiKey))
            {
                if (ret.ApiKey.Length > 4) ret.ApiKey = new string('*', 8) + ret.ApiKey.Substring(ret.ApiKey.Length - 4);
                else ret.ApiKey = new string('*', 8);
            }
            return ret;
        }

        /// <summary>
        /// Check whether a supplied API key value is a redacted placeholder produced by Redact.
        /// </summary>
        /// <param name="value">API key value.</param>
        /// <returns>True if the value is a redacted placeholder.</returns>
        public static bool IsRedactedApiKey(string value)
        {
