namespace LiteGraph.GraphRepositories.Postgresql.Implementations
{
    ...
    public class ChatThreadMethods : IChatThreadMethods
    {
        ...
        public ChatThreadMethods(PostgresqlGraphRepository repo)
        {
        ...
        public async Task<ChatThread> Create(ChatThread thread, CancellationToken token = default)
        {
            ...
            DataTable createResult = await _Repo.ExecuteQueryAsync(ChatThreadQueries.Insert(thread), true, token).ConfigureAwait(false);
            return Converters.ChatThreadFromDataRow(createResult.Rows[0]);
        }
            ...
            [EnumeratorCancellation] CancellationToken token = default)
        {
            ...
            {
                token.ThrowIfCancellationRequested();
                DataTable result = await _Repo.ExecuteQueryAsync(ChatThreadQueries.SelectAllInTenant(
                    tenantGuid,
                    userGuid,
                ...
                {
                    token.ThrowIfCancellationRequested();
                    yield return Converters.ChatThreadFromDataRow(result.Rows[i]);
                    skip++;
                }
        ...
        {
            token.ThrowIfCancellationRequested();
            DataTable result = await _Repo.ExecuteQueryAsync(ChatThreadQueries.SelectByGuid(tenantGuid, guid), false, token).ConfigureAwait(false);
            if (result != null && result.Rows.Count == 1) return Converters.ChatThreadFromDataRow(result.Rows[0]);
            return null;
        }
        ...
        public async Task<EnumerationResult<ChatThread>> Enumerate(EnumerationRequest query, CancellationToken token = default)
        {
            ...
            EnumerationResult<ChatThread> ret = new EnumerationResult<ChatThread>
            {
            ...
            token.ThrowIfCancellationRequested();
            DataTable result = await _Repo.ExecuteQueryAsync(ChatThreadQueries.GetRecordPage(
                query.TenantGUID,
                query.UserGUID,
                query.MaxResults,
                query.Skip,
            ...
            ret.Objects = Converters.ChatThreadsFromDataTable(result);

            ChatThread lastItem = ret.Objects.Last();
            ...
            CancellationToken token = default)
        {
            ...
            token.ThrowIfCancellationRequested();
            DataTable result = await _Repo.ExecuteQueryAsync(ChatThreadQueries.GetRecordCount(
                tenantGuid,
                userGuid,
                order,
                marker), false, token).ConfigureAwait(false);
        ...
        public async Task<ChatThread> Update(ChatThread thread, CancellationToken token = default)
        {
            if (thread == null) throw new ArgumentNullException(nameof(thread));
            token.ThrowIfCancellationRequested();
            DataTable result = await _Repo.ExecuteQueryAsync(ChatThreadQueries.Update(thread), true, token).ConfigureAwait(false);
            return Converters.ChatThreadFromDataRow(result.Rows[0]);
        }
        ...
        {
            token.ThrowIfCancellationRequested();
            await _Repo.ExecuteQueryAsync(ChatThreadQueries.Delete(tenantGuid, guid), true, token).ConfigureAwait(false);
        }
        ...
        {
            token.ThrowIfCancellationRequested();
            await _Repo.ExecuteQueryAsync(ChatThreadQueries.DeleteAllInTenant(tenantGuid), true, token).ConfigureAwait(false);
        }
        ...
        {
            token.ThrowIfCancellationRequested();
            ChatThread thread = await ReadByGuid(tenantGuid, guid, token).ConfigureAwait(false);
            return (thread != null);
        }
