...
- **#14 — No multi-`MATCH` query chaining (score 18).** Closed. A read query can now chain multiple `MATCH` clauses, optionally separated by `WITH` stages, terminated by one `RETURN`; later clauses reference earlier-bound variables and join on them. `WITH` projects/re-scopes, filters (HAVING, including on aggregate aliases), orders, skips, limits, and aggregates with grouping (`COUNT`/`SUM`/`AVG`/`MIN`/`MAX`). This turns the single-pattern query language into a genuine pipeline — multi-step reasoning ("who does someone I know work with," grouped counts, filtered projections) is now expressible in one query instead of stitched together in application code. Intermediate join/grouping sets reuse the #6 `MaxScanRows` ceiling (rejected, not truncated, on overflow). Chained `MATCH` clauses are node/single-edge (multi-hop via successive clauses); variable-length, `MATCH SHORTEST`, and `OPTIONAL MATCH` inside a chain are deferred. Delivered across the C# SDK, REST, and MCP, with a dual-storage Touchstone case covering joins, cross-clause WHERE, WITH projection/filter/order, grouping + HAVING, and negative cases.
...
- Reasoning correctness/shape: **#11 32-hop cap**.
...
| 14 | No multi-`MATCH` query chaining (§3.3) | Query chaining not yet supported | 7 | 6 | 5 | 18 | ✅ **Fixed (v9.0)** | Done |
...
### 14. No multi-`MATCH` query chaining (§3.3) — Score 18 — ✅ Fixed (v9.0)

From the traversal/query table, LiteGraph row:

> Query chaining (multi-`MATCH`) — **Not yet supported**

**Resolution (v9.0).** The query language is now a pipeline. A read query chains multiple `MATCH` clauses (optionally separated by `WITH`) terminated by one `RETURN`; later clauses reference earlier-bound variables and join on them, so multi-step patterns like *"companies worked at by people someone knows"* are one query. `WITH` projects/re-scopes variables, filters (HAVING, including on aggregate aliases), orders/skips/limits the intermediate stream, and aggregates with grouping. Chained `MATCH` clauses cover a node or a single directed edge (multi-hop is expressed as successive clauses); variable-length, `MATCH SHORTEST`, and `OPTIONAL MATCH` inside a chain are deferred to a follow-on. Intermediate join and grouping sets reuse the `MaxScanRows` ceiling from #6 — rejected with a `400` rather than truncated on overflow. Delivered across the C# SDK, REST, and MCP; single-clause queries are unchanged. See [docs/DSL.md](docs/DSL.md#query-chaining-multiple-match-and-with). Validated by the dual-storage `Query.Chaining.*` Touchstone case (joins, cross-clause WHERE, WITH projection/filter/order, grouping + HAVING, and negative cases).
